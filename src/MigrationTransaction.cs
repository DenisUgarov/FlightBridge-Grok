...
public static class MigrationTransaction {
 ...
 static string ExecuteCore(AutomaticPlan plan,string backupRoot,Action guard,Action<int> afterWrite){
  ...
  var journal=new XDocument(new XElement("Transaction",new XAttribute("Version","1"),new XAttribute("AppVersion",BuildInfo.Version),new XAttribute("State","Preparing")));string manifest=Path.Combine(session,"transaction.xml");
