...
2. Если найдено несколько аккаунтов Steam с профилями обеих игр — выберите
   нужный аккаунт в списке.
3. Если в MSFS 2024 ещё нет сохранённого профиля устройства, следуйте
   пошаговой подсказке на экране (сохраните профиль в игре, закройте симулятор
   и Steam, нажмите **«Проверить снова»**).
4. На экране предпросмотра проверьте таблицу: профиль 2020 → профиль 2024,
   число назначений и осей, список пропущенных команд с причиной.
5. Основное действие — **«Сохранить файл для импорта в игре»**. Программа
   записывает XML через безопасный экспорт (`Engine.Export` /
   `Library.ExportBatch`), открывает папку и показывает шаги импорта:
   MSFS 2024 → Параметры → Управление → то же устройство → та же категория →
   Импорт.
6. Прямая запись в игровые хранилища (прежнее поведение
   `MigrationTransaction.Execute`) скрыта за переключателем **«Дополнительно»**
   и требует красного подтверждения с перечнем перезаписываемых профилей;
   перед записью по-прежнему создаётся полный бэкап.
7. Полностью закройте MSFS 2020, MSFS 2024 и Steam перед экспортом или
   прямой записью. После импорта в игре проверьте оси и кнопки перед полётом.

Если версия 0.5.0 уже выполняла перенос на этом компьютере, версия 0.5.1 сама
обнаружит его резервную копию. Восстановление из последней копии по-прежнему
доступно отдельной кнопкой.

Выбирать технические XML-параметры не требуется. Если профиль нельзя
сопоставить однозначно, он остаётся без изменений и попадает в список
пропусков на предпросмотре. Программа не угадывает и не создаёт
незарегистрированные облачные записи.

`Main.xaml` удалён: интерфейс собирается в коде (`AutomaticApp`).
...
Рекомендуемый путь — экспорт файла для импорта в игре: живые remote/wgs
хранилища при этом не переписываются. При выборе прямой записи из раздела
«Дополнительно» Flight Bridge обязательно копирует целиком исходное и целевое
облачное хранилище, вычисляет SHA-256 каждого файла и повторно проверяет, что
данные игры не изменились во время подготовки. Отключить бэкап для прямой
записи нельзя.
