...
4. Когда план готов, откроется предпросмотр: куда что переносится, что
   ...
   контекст).
5. Основная кнопка — **«Перенести настройки»**: запись в уже зарегистрированные
   профили MSFS 2024 (`MigrationTransaction.Execute` через адаптер
   `WriteToGame`). Перед записью проверяется, что MSFS 2020/2024 и Steam
   закрыты; затем одно обычное подтверждение. Полный бэкап создаётся
   автоматически (по умолчанию в Документы; сменить диск можно по ссылке внизу,
