...
try {
 ...
 & $compiler /nologo /target:exe /out:dist\UpdaterTests.exe /r:System.dll /r:System.Core.dll obj\VersionInfo.cs src\Updater.cs tests\UpdaterTests.cs
 if ($LASTEXITCODE -ne 0) { throw 'Updater test build failed' }
 & .\dist\UpdaterTests.exe
...
foreach ($name in @('CoreTests.exe','AutoMigrationTests.exe','AppLocalizationTests.exe','PreviewModelTests.exe','UpdaterTests.exe','DeviceCheckTests.exe','FlightBridge-Diagnostics.exe')) {
